    ...
    stages {
        stage("Clone Git Repo") {
            steps {
                ...
                checkout([
                    ...
                    extensions: [
                        [$class: 'RelativeTargetDirectory', relativeTargetDir: 'v8.go'],
                        [$class: 'CloneOption', shallow: true, depth: 1, noTags: false]
                    ],
                    submoduleCfg: [],
                    userRemoteConfigs: [[credentialsId: 'github-app-podio-jm', url: 'https://github.com/podio/v8.go.git']]
                ])
            }
        }
        ...
        stage('Polaris') {
            environment {
                ...
                BRIDGE_POLARIS_PROJECT_NAME = "v8.go"
                BRIDGE_POLARIS_BRANCH_NAME = "${env.branchName}"
                BRIDGE_POLARIS_ASSESSMENT_TYPES = "SAST"
                BRIDGE_POLARIS_COVERITY_CAPTURE_BUILD_COMMAND = "go build ./..."
            }
            steps {
                dir('v8.go') {
                    script {
        ...
        stage('BlackDuckSCA') {
            environment {
                ...
                BRIDGE_DETECT_ARGS = "--detect.project.name=DX-Podio-v8.go --detect.project.version.name=${env.branchName} --detect.project.version.update=true --detect.project.version.distribution=SAAS --detect.project.group.name=Podio-Podio"
            }
            steps {
                dir('v8.go') {
                    script {
